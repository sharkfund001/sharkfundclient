...
void CAddrBook::OnBnClickedButtonAddaddrbook()
{
	...
	if (nResponse == IDOK)
	{
		...
		strCommand = strprintf("%s %s", "validateaddress", addr.address.c_str());
...
BOOL CAddrBook::OnInitDialog()
{
	...
	m_listCtrl.SetHeaderBKColor(226,226,226,0/*224,65,18,0*/); 
	...
	m_listCtrl.SetWindowPos( NULL , 0 ,28 , 790, 270  ,SWP_SHOWWINDOW ) ; 

	//m_rBtnClose.SetBitmaps( IDB_BITMAP_CLOSE1 , RGB(255, 255, 0) , IDB_BITMAP_CLOSE2 , RGB(255, 255, 255) );
	...
	*/
	...
	m_rBtnAdd.SetBitmaps( IDB_BITMAP_BUT1 , RGB(255, 255, 0) , IDB_BITMAP_BUT1 , RGB(255, 255, 255) );
	m_rBtnAdd.SetAlign(CButtonST::ST_ALIGN_OVERLAP);
	m_rBtnAdd.SetWindowText("添加地址") ;
	m_rBtnAdd.SetFontEx(-12 , _T("微软雅黑"));
	m_rBtnAdd.SetColor(CButtonST::BTNST_COLOR_FG_OUT , RGB(255, 255, 255));
	m_rBtnAdd.SetColor(CButtonST::BTNST_COLOR_FG_IN , RGB(200, 75, 60));
	m_rBtnAdd.SetColor(CButtonST::BTNST_COLOR_FG_FOCUS, RGB(255, 255, 255));
	m_rBtnAdd.SetColor(CButtonST::BTNST_COLOR_BK_IN, RGB(255, 255, 255));
	m_rBtnAdd.SizeToContent();
	//m_rBtnAdd.SetWindowPos(NULL ,80 , 100, 0 , 0 , SWP_NOSIZE); 

	m_rBtnDelete.SetBitmaps( IDB_BITMAP_ORANGE_BUT1 , RGB(255, 255, 0) , IDB_BITMAP_ORANGE_BUT1 , RGB(255, 255, 255) );
	m_rBtnDelete.SetAlign(CButtonST::ST_ALIGN_OVERLAP);
	m_rBtnDelete.SetWindowText("删除地址") ;
	m_rBtnDelete.SetFontEx(-12 , _T("微软雅黑"));
	m_rBtnDelete.SetColor(CButtonST::BTNST_COLOR_FG_OUT , RGB(255, 255, 255));
	m_rBtnDelete.SetColor(CButtonST::BTNST_COLOR_FG_IN , RGB(200, 75, 60));
	m_rBtnDelete.SetColor(CButtonST::BTNST_COLOR_FG_FOCUS, RGB(255, 255, 255));
	m_rBtnDelete.SetColor(CButtonST::BTNST_COLOR_BK_IN, RGB(255, 255, 255));
	m_rBtnDelete.SizeToContent();
	//m_rBtnDelete.SetWindowPos(NULL ,240 , 100, 0 , 0 , SWP_NOSIZE); 

	m_rBtnOk.SetBitmaps( IDB_BITMAP_BUT1 , RGB(255, 255, 0) , IDB_BITMAP_BUT1 , RGB(255, 255, 255) );
	m_rBtnOk.SetAlign(CButtonST::ST_ALIGN_OVERLAP);
	m_rBtnOk.SetWindowText("确定") ;
	...
	//m_rBtnOk.SetWindowPos(NULL ,600 , 100, 0 , 0 , SWP_NOSIZE); 
