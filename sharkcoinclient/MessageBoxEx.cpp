...
*/
void CMessageBoxEx::SetIcon(UINT uIconResource)
{
	HICON hIconIn = (HICON)::LoadImage(AfxGetInstanceHandle(), MAKEINTRESOURCE(uIconResource), IMAGE_ICON, 0, 0, 0);
	m_Image.SetIcon(hIconIn);
	m_Image.DrawBorder(FALSE); 
	m_Image.DrawTransparent(TRUE);
}
...
//	m_strText.SetWindowText(m_Text);
	...
	/*
	m_quit.SetIcon(IDI_ICON_CLOSE3, 0, 0, IDI_ICON_CLOSE1, 0, 0);
	m_quit.DrawBorder(FALSE); 
	m_quit.DrawTransparent(TRUE);
	m_quit.SizeToContent();
	*/
	...
	RECT ret;
	GetWindowRect(&ret);
	m_quit.SetWindowPos(NULL ,(ret.right-ret.left)-titleRect.Width() - 4, -1 , 0 , 0 , SWP_NOSIZE); 

	//图片
	if ( (m_uType & MFB_TIP) == MFB_TIP ) {
		//SetBitmap(IDB_BITMAP_TIP);
		SetIcon(IDI_ICON_TIP);
	}else if ( (m_uType & MFB_ERROR) == MFB_ERROR ) {
		//SetBitmap(IDB_BITMAP_ERROR);
		SetIcon(IDI_ICON_ERROR);
	}else if ( (m_uType & MFB_WARNING) == MFB_WARNING ) {
		//SetBitmap(IDB_BITMAP_WARNING);
		SetIcon(IDI_ICON_WARNING);
	}
	GetDlgItem(IDC_IMAGE)->MoveWindow(nIconLeftMargin + 10 , nIconTopMargin + 2 , 41 , 41);
	...
	//显示文本的位置
	m_strText.SetFont(90, _T("微软雅黑"));				//设置显示字体和大小
	m_strText.SetTextColor(RGB(252,144,5));	
	m_strText.MoveWindow(nLeftMargin, nIconTopMargin+15, nStaticWidth, nStaticHeigh);
	m_strText.SetWindowText(m_Text);
	//m_Text.SetTextColor(RGB(254,205,165));

	if ( (m_uType & MFB_OKCANCEL) == MFB_OKCANCEL ) {
		  m_OK.SetBitmaps( IDB_BITMAP_BUT1 , RGB(255, 255, 0) , IDB_BITMAP_BUT1 , RGB(255, 255, 255) );
		   ...
		   */
		   ...
		   m_OK.SizeToContent();
		   m_OK.MoveWindow(/*nDialogWidth-nBtRightMargin-56-56-56-10*/nLeftMargin, nDialogHight-nBtBottomMargin-23, 56, 23);
		   

		   m_cancel.SetBitmaps( IDB_BITMAP_ORANGE_BUT1 , RGB(255, 255, 0) , IDB_BITMAP_ORANGE_BUT1 , RGB(255, 255, 255) );
		   ...
		   */
		   ...
		   //否按钮位置
		   m_cancel.MoveWindow(/*nDialogWidth-nBtRightMargin-56-56*/nLeftMargin + 66,nDialogHight-nBtBottomMargin-23, 56, 23);
	}
		   ...
		   */
		   ...
		   m_OK.MoveWindow(nDialogWidth -56 - nStaticWidth, nDialogHight-nBtBottomMargin-23,56,23);
