...
BEGIN_MESSAGE_MAP(CDeal, CDialogBar)
	...
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_BUTTON_SEARCH, &CDeal::OnBnClickedButtonSearch)
END_MESSAGE_MAP()
...
void CDeal::OnSize(UINT nType, int cx, int cy)
{
	...
	if( NULL != GetSafeHwnd() ) {
		...
		pst =  GetDlgItem(IDC_STATIC_DEAL_GROUP);
		if(NULL != pst)
		{
		...
		if ( NULL != pst ) {
			pst->SetWindowPos( NULL ,9 , 100 , 880, 319,SWP_SHOWWINDOW ) ; 
		}

		pst = GetDlgItem( IDC_COMBO_TIME ) ;
		if ( NULL != pst ) {
			pst->GetClientRect( rect ) ;
			pst->SetWindowPos( NULL ,15 , 36 , 128, rect.Height()  ,SWP_SHOWWINDOW ) ; 
		}
		pst = GetDlgItem( IDC_COMBO1 ) ;
		if ( NULL != pst ) {
			pst->GetClientRect( rect ) ;
			pst->SetWindowPos( NULL ,15+128+20 , 36 , 128, rect.Height()  ,SWP_SHOWWINDOW ) ; 
		}
		pst = GetDlgItem( IDC_EDIT_SEARCH ) ;
		if ( NULL != pst ) {
			pst->GetWindowRect( rect ) ;
			pst->SetWindowPos( NULL ,15+15+20+128+128 , 36 , 250, rect.Height()  ,SWP_SHOWWINDOW ) ; 
		}
		...
		if(NULL != pst)
		{
			...
			pst->SetWindowPos(NULL, 15+15+20+128+128 + 260, 34, btnRect.Width(), btnRect.Height(), SWP_SHOWWINDOW);
		}

		pst = GetDlgItem( IDC_STATIC_PAGE ) ;
		if ( NULL != pst ) {
			pst->SetWindowPos( NULL ,10 , 430 , 30, 24  ,SWP_SHOWWINDOW ) ; 
		}
		pst = GetDlgItem( IDC_COMBO_PAGE ) ;
		if ( NULL != pst ) {
			pst->SetWindowPos( NULL ,45 , 426 ,  72, rect.Height() ,SWP_SHOWWINDOW ) ; 
		}
		pst = GetDlgItem( IDC_STATIC_TM ) ;
		if ( NULL != pst ) {
			pst->SetWindowPos( NULL ,125 , 430 , 30, 24  ,SWP_SHOWWINDOW ) ; 
		}
		pst = GetDlgItem( IDC_UP_PAGE ) ;
		if ( NULL != pst ) {
			pst->GetClientRect( rect ) ;
			pst->SetWindowPos( NULL ,160 , 432 , 13, 13  ,SWP_SHOWWINDOW ) ; 
		}
		pst = GetDlgItem( IDC_EDIT_PAGE ) ;
		if ( NULL != pst ) {
			pst->SetWindowPos( NULL ,185 , 430 , 30, 18  ,SWP_SHOWWINDOW ) ; 
		}
		pst = GetDlgItem( IDC_NEXT_PAGE ) ;
		if ( NULL != pst ) {
			pst->GetClientRect( rect ) ;
			pst->SetWindowPos( NULL ,220 , 432 , 13, 13  ,SWP_SHOWWINDOW ) ; 
		}
		pst = GetDlgItem( IDC_STATIC_SUM ) ;
		if ( NULL != pst ) {
			pst->SetWindowPos( NULL ,220 , 430 , 30, 24  ,SWP_SHOWWINDOW ) ; 
		}

		pst = GetDlgItem( IDC_STATIC_COUNT_PAGE ) ;
		if ( NULL != pst ) {
			pst->SetWindowPos( NULL ,250 , 430 , 60, 24  ,SWP_SHOWWINDOW ) ; 
		}

		pst = GetDlgItem( IDC_EXPORT_EXEL ) ;
		if ( NULL != pst ) {
		   pst->GetWindowRect(rect);
		   pst->SetWindowPos( NULL ,900 - rect.Width() * 2 - 20 ,  426  , rect.Width(), rect.Height()  ,SWP_SHOWWINDOW ) ; 
		}
		pst = GetDlgItem( IDC_BUTTON_TXDETAIL ) ;
		if ( NULL != pst ) {
			pst->GetWindowRect(rect);
			pst->SetWindowPos( NULL ,900 - rect.Width() - 10 ,  426  , rect.Width(), rect.Height()  ,SWP_SHOWWINDOW ) ; 
		}
	}
}
...
BOOL CDeal::Create(CWnd* pParentWnd, UINT nIDTemplate, UINT nStyle, UINT nID)
{
	...
	if ( bRes ) {
		...
		m_rBtnTxdetail.SetBitmaps( IDB_BITMAP_BUT1 , RGB(255, 255, 0) , IDB_BITMAP_BUT1 , RGB(255, 255, 255) );
		m_rBtnTxdetail.SetAlign(CButtonST::ST_ALIGN_OVERLAP);
		m_rBtnTxdetail.SetWindowText("查看详情") ;
		m_rBtnTxdetail.SetFontEx(-12 , _T("微软雅黑"));
		...
		m_rBtnExportTx.SetBitmaps( IDB_BITMAP_BUT1 , RGB(255, 255, 0) , IDB_BITMAP_BUT1 , RGB(255, 255, 255) );
		m_rBtnExportTx.SetAlign(CButtonST::ST_ALIGN_OVERLAP);
		m_rBtnExportTx.SetWindowText("导出Excel") ;
		m_rBtnExportTx.SetFontEx(-12 , _T("微软雅黑"));
		...
		m_rBtnSearch.SetBitmaps( IDB_BITMAP_BUT1 , RGB(255, 255, 0) , IDB_BITMAP_BUT1 , RGB(255, 255, 255) );
		...
		m_rBtnSearch.SetColor(CButtonST::BTNST_COLOR_FG_IN , RGB(200, 75, 60));
		...
		m_rBtnUp.SetBitmaps( IDB_TRADE_NEXT , RGB(255, 255, 0) , IDB_TRADE_NEXT , RGB(255, 255, 255) );
		...
		m_rBtnNext.SetBitmaps( IDB_TRADE_UP , RGB(255, 255, 0) , IDB_TRADE_UP , RGB(255, 255, 255) );
...
void CDeal::OnBnClickedButtonSearch()
{
	// TODO: 在此添加控件通知处理程序代码
	ShowAddrConditon();
}
