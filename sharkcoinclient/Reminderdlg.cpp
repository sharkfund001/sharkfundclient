...
void CReminderdlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_STATIC_REMIND_SHOW, m_Text);
	DDX_Control(pDX, IDC_REMIND_BTN_CLOSE, m_rBtnClose);	
...
BEGIN_MESSAGE_MAP(CReminderdlg, CDialogEx)
	...
	ON_WM_ERASEBKGND()
	ON_BN_CLICKED(IDC_REMIND_BTN_CLOSE, &CReminderdlg::OnBnClickedButtonClose)
	ON_WM_SIZE()
END_MESSAGE_MAP()
...
BOOL CReminderdlg::OnInitDialog()
{
	...
	m_Text.SetWindowPos( NULL , 5 , 4 , 100, 25  ,SWP_SHOWWINDOW ) ; 
	m_Text.SetWindowText(_T("关闭提示"));
	...
	*/
	...
	GetDlgItem(IDC_RADIO_NOEXIT)->SetWindowText(_T("最小化到系统托盘区,不退出程序"));
	GetDlgItem(IDC_RADIO_EXIT)->SetWindowText(_T("退出程序"));
	GetDlgItem(IDC_CHECK_NO)->SetWindowText(_T("不再提醒此对话框"));
...
void CReminderdlg::OnBnClickedOk()
{
	...
	if (((CButton *)GetDlgItem(IDC_RADIO_NOEXIT))->GetCheck())
	{
	   ...
	   ((CsharkcoinclientDlg*)(theApp.m_pMainWnd))->ToTray();
